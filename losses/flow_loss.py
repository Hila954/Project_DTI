...
from utils.flow_utils import flow_warp, get_occu_mask_bidirection
from utils.misc import log

# Loss blocks
from losses.NCC import NCC
from losses.mask_loss import MaskWarpLoss
from losses.cyclic_loss import CyclicLoss
from losses.kpts_loss import KptsLoss

def get_loss(args):
    if "ncc" in args.loss:
        losses = {"loss_module" : NCCLoss(args)}
    else:
        losses = {"loss_module" : UnFlowLoss(args)}

    if "mwl" in args.loss:
        losses.update({"mwl_module" : MaskWarpLoss(args)})

    if 'cyc' in args.loss:
        losses.update({"cyc_module" : CyclicLoss(args)})

    if 'kpts' in args.loss:
        losses.update({"kpts_module" : KptsLoss(args)})
...
class NCCLoss(nn.modules.Module):
    ...
    def forward(self, output, img1, img2, aux, vox_dim):
        ...
        pyramid_occu_masks = [(occu1,occu2) for occu1,occu2 in zip(pyramid_occu_mask1,pyramid_occu_mask2)]

        return loss_total, loss_ncc, loss_smooth, loss_admm, pyramid_flows[0].abs().mean(), pyramid_occu_masks
