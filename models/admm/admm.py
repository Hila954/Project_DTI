...
class ADMMSolverBlock(nn.Module):
    ...
    def forward(self, F, masks, vox_dim):
        ...
        if self.grad == "2nd":
            dF2 = [self.get_gradients(df, vox_dim) for df in dF] 
...
class MaskGenerator(nn.Module):
    ...
    #* deterministic importance matrix, aiming to increase the penalty on the boundaries (W from costunrolling)
    def forward(self, image, vox_dim, scale=1/8): 
        if self.learn_mask: 
            im_grads = self.sobel(image,vox_dim) #[dx, dy, dz] **of image** 
            encoders = [self.ddx_encoder, self.ddy_encoder, self.ddz_encoder] #****model with conv** (not me)
