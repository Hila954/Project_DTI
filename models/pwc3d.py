...
class PWC3D(nn.Module):
    def __init__(self, args, upsample=True, reduce_dense=True, search_range=4):
        ...
        #~ change first parameter to 6 
        if args.train_type == "DTI_Example_train":
            self.num_chs = [6, 16, 32, 64, 96, 128, 192]
        else:
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, vox_dim, w_bk=True):
        ...
        #masks.append(self.mask_gen(x1, vox_dim, scale=1/4))
        #masks.append(self.mask_gen(x2, vox_dim, scale=1/4))

        log(f'Pyramidized inputs')
        res_dict={}
        res_dict['flows_fw'] = self.forward_2_frames(x1_p, x2_p, mask=masks[0], vox_dim=vox_dim)
        if w_bk:
            res_dict['flows_bk'] = self.forward_2_frames(x2_p, x1_p, mask=masks[1], vox_dim=vox_dim)
        return res_dict

    def forward_2_frames(self, x1_p: torch.Tensor, x2_p: torch.Tensor, mask, vox_dim):
        # init
        flows = []
        aux_vars = {
                "q":        [],
        ...
        for l, (_x1, _x2) in enumerate(zip(x1_p, x2_p)):
            ...
            if self.apply_admm[l]:
                vox_dim_sc = vox_dim / 2**(self.num_levels - l - 1)
